...
@Component({
  selector: 'app-view-template4',
  templateUrl: './view-template4.component.html',
  styleUrls: ['./view-template4.component.scss']
})
export class ViewTemplate4Component implements OnInit {
  ...
  download() {
    ...
    this.cardService.getCardLimitById(this.uid)
      .subscribe((result) => {
       this.cardLimit = result;
       console.log('card limit is' + this.cardLimit );
      },
        (error) => {
          alert('Failed to load');
        });
    ...
    if (this.cardLimit > 0) {
      ...
      window.alert("you exceed your limit");
    }
  }
